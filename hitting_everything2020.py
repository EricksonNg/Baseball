...
def hitting_everything2020():
  sched = statsapi.schedule(start_date='',end_date='',team=137)
          ...
          if ID in scoredata['home']['players']:
            ...
            if scoredata['home']['players'][ID]['stats']['batting']!={}:
              if scoredata['home']['players'][ID]['position']['abbreviation']!='P':
                h_add(game_date,sched,scoredata,ID) 
        else:
          if ID in scoredata['away']['players']:
            ...
            if scoredata['away']['players'][ID]['stats']['batting']!={}:
              if scoredata['away']['players'][ID]['position']['abbreviation']!='P':
                a_add(game_date,sched,scoredata,ID)

def h_add(game_date, sched, scoredata, ID):
  ...
  if path.exists("2020/"+playername+".txt")==True:
    ...
    with open("2020/"+str(playername)+".txt", "w") as f:
      f.write("{'"+playername+"': {'ID':'"+ID+"', '2020': {'dates': [], 'progression': {'averages': [], 'obp': [], 'slg': [], 'ops': [], 'runs': [], 'doubles': [], 'triples': [], 'homeruns': [], 'strikeouts': [], 'walks': [], 'hits': [], 'ab': [], 'sb': [], 'rbi': [], 'lob': []}, 'per_game': {'ab': [], 'strikeouts': [], 'hits': [], 'walks': [], 'runs': [], 'rbi': [], 'sb': [], 'lob': [], 'doubles': [], 'triples': [], 'homeruns': []}}}}")
    with open("2020/"+str(playername)+".txt", "r") as FILE:
      content = FILE.read()
      content_dict = eval( content )
...
def a_add(game_date, sched, scoredata, ID):
