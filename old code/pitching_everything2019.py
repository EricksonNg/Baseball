...
def h_add(game_date, scoredata, ID):
    ...
    if game_date not in content_dict[playername]['2019']['pitching']['dates']:
        print("Yes")
        with open("2019/" + str(playername) + ".txt", "w") as f:
            try:
                content_dict[playername]['2019']['pitching']['dates'].append(game_date)
                for i in range(len(p_categories)):
                    content_dict[playername]['2019']['pitching']['progression'][p_names[i]].append(p_categories[i])
                for i in range(len(pg_categories)):
                    content_dict[playername]['2019']['pitching']['per_game'][pg_names[i]].append(pg_categories[i])
